...
class DataScraperService:
    ...
    def __init__(
        ...
        self.seed_service = seed_service or SeedDiscoveryService(self.match_repo.api_client, self.summoner_repo)
    ...
    async def scrape_matches_by_date_window(
        ...
        if not puuid_queue:
            ...
            if not puuid_queue:
                puuid_queue = await self.seed_service.discover_seed_puuids(region, queue_type, count=5)
            if not puuid_queue:
        ...
        while puuid_queue:
            ...
            if not puuid_queue and (max_matches is None or len(matches) < max_matches):
                extra = await self.seed_service.discover_seed_puuids(region, queue_type, count=50)
                for pu in extra:
